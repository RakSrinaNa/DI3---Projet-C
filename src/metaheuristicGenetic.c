...
Solution * metaheuristicGenetic_search(Instance * instance, SolutionType solutionType, int populationMaxSize, float mutationProbability, int maxIterations, int styleNaturalSelection, int styleParentSelection)
{
	//Random
	srand(time(NULL));
	...
	while(i < maxIterations)
	{
		...
		for(int j = 0; j < populationMaxSize / 2; j++)
		{
			...
			metaheuristicGenetic_selectParents(population, &parent1, &parent2, styleParentSelection);
...
void metaheuristicGenetic_selectParentsFight(Population * population, Solution ** parent1, Solution ** parent2)
{
	...
	//Selectionne le meilleur des deux combattants pour définir lequel sera le parent 1
	...
	//Selectionne le meilleur des deux combattants pour définir lequel sera le parent 2
